...
- **تست:** pytest.
- **دیتابیس:** در MVP بدون دیتابیس (stateless، ورودی/خروجی JSON)؛ در فاز ۱ اتصال به Postgres برای ذخیره پروفایل مشتری و CampaignHistory.
- **استقرار (فاز بعد):** طبق راهنمای بیلد پروژه، بیلد نهایی روی سرور SSH انجام می‌شود، نه داخل کانتینر توسعه.
